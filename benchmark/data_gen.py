#!/usr/bin/env python3
"""
benchmark/data_gen.py: Generate the synthetic "Island" dataset for the
head-to-head benchmark.

MariaDB port of fractalsql-postgresql's bench/data_gen.py. Creates N
...
major in the 10.6-12.2 compat floor this repo targets. --with-native-
...
VECTOR INDEX on it -- this repo's actual ANN-index comparison arm,
replacing postgres's pgvector/HNSW. Default off so the base
head_to_head.py run (Scout Mode vs. brute-force distance-sort) is
unaffected on majors without native VECTOR(n) support.

There is no MariaDB equivalent of postgres's own custom `fractal_vector`
SQL type at all (no CREATE TYPE / typmod mechanism in MariaDB) -- the
...
Usage:
    python3 benchmark/data_gen.py --host 127.0.0.1 --port 3306 \\
        --n 5000 --dim 128 --clusters 50

Defaults: 5000 points, dim=128, 50 clusters -- MUCH smaller than
postgres's 100k/dim=768 default. See benchmark/README.md's "Why such a
small default N" section: fractal_explore() (this repo's Scout Mode
port) takes its ENTIRE corpus as one client-supplied inline string
argument (no SPI / table-returning UDFs in MariaDB's C UDF ABI, so no
server-side table scan is possible for it at all, see sql/
install_udf.sql's own "Scout Mode" comment). At postgres's
100k x dim=768 scale that string would be hundreds of MB, sent over the
wire on every single query, impractical for a benchmark meant to
actually finish. benchmark/vector_type_head_to_head.py's
...
def main() -> int:
    ...
    ap.add_argument("--n", type=int, default=5_000,
                    help="number of points (default: %(default)s -- see this "
                         "file's own docstring for why postgres's 100k default "
