...
/* ----- Vector Arithmetic --------------------------------------------
 *
 * Simple O(dim) float32 vector ops, added to serve the float4-style
 * packed vector storage format directly, with no float<->double
 * conversion at the call site. Deliberately NOT used by
