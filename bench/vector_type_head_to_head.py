...
MariaDB has no way to define a custom SQL type
at all (no CREATE TYPE / type-modifier mechanism exists here) -- the two real
storage shapes here are the portable TEXT/JSON-array-string convention
(works on every MariaDB major in the 10.6-12.3 compat floor) and
MariaDB's own built-in VECTOR(n) (GA 11.7+ only). Distinct from
bench/head_to_head.py (native-index-vs-Scout-Mode algorithm
...
Measures:
  ...
  (d) Peak server-process RSS during a full-corpus scan: NOT PORTED, see
      ...
      per connection. MariaDB's threading model is the opposite: ONE
